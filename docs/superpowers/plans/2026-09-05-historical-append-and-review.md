...
- [x] Write failing behavioral tests before implementation. Use the existing production pipeline, not a mock. Initial repository test:
...
Final integration review found and corrected two issues: Policy Map tests were using the old generated snapshot and stale graph totals; editorial Record level and Version status appeared under Official metadata. Data-dependent tests now share the verified payload with the build, retaining focused graph invariants while checking 100 linked nodes and 95 edges. Both editorial fields now appear under Research classifications. The rebuilt historical and retained pending record pages were checked, and the rendered complete relationship list has 95 rows. Deferred non-blocking items are the three existing Astro hints and conservative scanner false positives for unusual URI-valid punctuation before a /home/ segment.
