...
The continuation on 5 September reviewed all 30 remaining records through additional official-source routes and admitted four: the adopted Parliament amendments, the content-approved transparency-guideline draft annex, and two Commission guidelines with later dated Service Desk manifestations. The current local canonical corpus contains 105 `verified` and 26 `legacy_review_pending` records. All 101 previously verified document files and all 131 route identities remain unchanged. The chronological ledger is `research/migrations/2026-09-05-review-continuation.json`.

The new-admission evidence gate is active. The 26 retained holds are the remaining measurable legacy-review queue: 23 lack exact-version publication evidence and three lack an attachment parent. The two user-approved compatibility corrections are resolved, not retained holds. Pending state does not mean an official document is fictitious or that its prior official-record checks failed. Completing a bounded search pass is not completing verification. Public UI/build verification and remote publication checks remain release gates distinct from metadata admission.
...
On the current corpus, the standalone all-document preflight remains nonzero because 26 retained legacy routes have not completed the extension. The continuation expects 131 checked and 105 ready; the fresh measured result is recorded in its ledger. That does not indicate a production-build regression: the compatibility gate validates the complete extension on reviewed records while preserving exact baseline ID/slug routes for pending legacy records. Three published draft-guideline section records carry retained-route notices; these explain why their stable routes remain available but do not suppress historical relationship issues or claim a parent relationship.
