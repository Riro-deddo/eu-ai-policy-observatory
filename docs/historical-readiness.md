...
The new-admission evidence gate is active. The 37 retained holds are the remaining measurable legacy-review queue: their pending state means the expanded date, role, classification or lineage evidence is incomplete, not that their prior official-record checks failed. Public UI/build verification and remote publication checks remain release gates distinct from metadata admission.
...
On the current corpus, the standalone all-document preflight remains nonzero because 37 retained legacy routes have not been migrated. That does not indicate a production-build regression: the compatibility gate validates the complete extension on reviewed records while preserving exact baseline ID/slug routes for pending legacy records. Three published draft-guideline section records carry retained-route notices; these explain why their stable routes remain available but do not suppress historical relationship issues or claim a parent relationship.
