...
On the current corpus, a nonzero preflight result is expected because the prospective fields have not been migrated. That result does not indicate a regression in the currently active build. Three published draft-guideline section records now carry active retained-route notices: these notices explain why their stable routes remain available, but deliberately do not suppress the historical relationship issues or claim a parent relationship.
