...
The Python suite checks schema, controlled vocabulary, composite identity, references, provenance, source-sweep and inventory contracts. `observatory-build` validates before atomically replacing generated outputs. Web tests and the public scanner check public-only rendering, routes, downloadable SQLite integrity, common credential markers and local-path leakage. The repository English guard scans Git-tracked UTF-8 text for CJK ideographs while allowing accented Latin names and machine identifiers.
